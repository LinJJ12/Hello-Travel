...
class TripRequest(BaseModel):
    ...
    destinations: List[str] = Field(default=[], description="多城市目的地列表", example=["杭州", "苏州", "上海"])
    ...
    budget_per_person: Optional[int] = Field(default=None, description="人均预算(元)", ge=0, example=3000)
    travel_pace: Optional[str] = Field(default="适中", description="旅行节奏: 轻松/适中/紧凑", example="适中")
    companions: Optional[str] = Field(default="独自/不限", description="同行人群", example="情侣")
    dietary_restrictions: Optional[str] = Field(default="", description="饮食禁忌或偏好", example="不吃海鲜")
...
class GraphNode(BaseModel):
    ...
    value: Optional[Union[int, float, str]] = Field(default=None, description="节点值")
